...
--
-- Helper/trigger functions are prefixed `cb_` (Capital Business) because this
-- project shares its Supabase database with an unrelated app — the prefix
-- avoids silently colliding with (or being overwritten by) a same-named
-- function the other app defines.

-- ---------------------------------------------------------------------------
-- helper functions (security definer so they can read `admins`/`workers`
-- without recursing into the RLS policies that call them)
-- ---------------------------------------------------------------------------
create or replace function cb_is_super_admin()
returns boolean
language sql
security definer
...
create or replace function cb_is_service_manager_for(target_service_type_id uuid)
returns boolean
language sql
security definer
set search_path = public
...
create or replace function cb_is_admin_for_service_type(target_service_type_id uuid)
returns boolean
language sql
security definer
set search_path = public
stable
as $$
  select cb_is_super_admin() or cb_is_service_manager_for(target_service_type_id);
$$;

create or replace function cb_owns_worker(target_worker_id uuid)
returns boolean
language sql
security definer
set search_path = public
...
create or replace function cb_worker_service_type_id(target_worker_id uuid)
returns uuid
language sql
security definer
set search_path = public
...
create or replace function cb_is_admin_for_worker(target_worker_id uuid)
returns boolean
language sql
security definer
set search_path = public
...
create or replace function cb_prevent_worker_service_type_change()
returns trigger
language plpgsql
as $$
begin
  if not cb_is_admin_for_service_type(new.service_type_id) and new.service_type_id <> old.service_type_id then
...
create trigger workers_prevent_service_type_change
  ...
  for each row execute function cb_prevent_worker_service_type_change();
...
create or replace function cb_prevent_review_content_edit_by_worker()
returns trigger
language plpgsql
as $$
begin
  if not cb_is_super_admin()
...
create trigger reviews_prevent_content_edit
  ...
  for each row execute function cb_prevent_review_content_edit_by_worker();
...
create policy admins_select on admins
  for select using (cb_is_super_admin() or user_id = auth.uid());

create policy admins_insert on admins
  for insert with check (cb_is_super_admin());

create policy admins_update on admins
  for update using (cb_is_super_admin());

create policy admins_delete on admins
  for delete using (cb_is_super_admin());
...
create policy workers_select on workers
  for select using (
    user_id = auth.uid()
    or cb_is_admin_for_service_type(service_type_id)
...
create policy workers_insert on workers
  for insert with check (cb_is_admin_for_service_type(service_type_id));

create policy workers_update on workers
  for update using (
    user_id = auth.uid() or cb_is_admin_for_service_type(service_type_id)
  );

create policy workers_delete on workers
  for delete using (cb_is_admin_for_service_type(service_type_id));
...
create policy worker_module_data_delete on worker_module_data
  for delete using (cb_is_admin_for_worker(worker_id));
...
create policy bookings_update on bookings
  for update using (
    cb_owns_worker(worker_id) or cb_is_admin_for_worker(worker_id)
  );
...
create policy reviews_select on reviews
  for select using (
    cb_is_publicly_visible_worker(worker_id)
    or cb_owns_worker(worker_id)
    or cb_is_admin_for_worker(worker_id)
...
create policy reviews_update on reviews
  for update using (
    cb_owns_worker(worker_id) or cb_is_super_admin()
  );
